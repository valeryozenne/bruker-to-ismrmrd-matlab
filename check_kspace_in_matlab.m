...
% let's check first the fid at the center of the ksapce for each contrast
...
%figure is of , fid are different
figure(1)
plot(abs(fid_c1(:,1)));
hold on;
plot(abs(fid_c2(:,1)));
...
str_msg=sprintf('le nombre de lignes ACQ_IS_IMAGE_contrast1 est  %d \n', size(acqs_image_c2,2)); disp(str_msg);
 ...
 % now I have two matrix  , do the ifft
...
subplot(231); imagesc(abs(mat.image.siemens(:,:,s,1))*1e6);  colormap(gray); title('echo1')
subplot(232); imagesc(abs(mat.image.siemens2(:,:,s,1))*1e6);  colormap(gray);  title('echo2')
subplot(233); imagesc(abs(mat.image.siemens(:,:,s,1))*1e6-abs(mat.image.siemens2(:,:,s,1))*1e6);  colormap(gray); title('substraction')
subplot(234); imagesc(angle(mat.image.siemens(:,:,s,1)));  colormap(gray); title('echo1')
subplot(235); imagesc(angle(mat.image.siemens2(:,:,s,1)));  colormap(gray);  title('echo2')
subplot(236); imagesc(angle(mat.image.siemens(:,:,s,1))-angle(mat.image.siemens2(:,:,s,1)));  colormap(gray); title('substraction')
pause(0.05)
end
