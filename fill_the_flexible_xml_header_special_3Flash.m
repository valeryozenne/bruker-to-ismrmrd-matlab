function [ header ] = fill_the_flexible_xml_header_special_3Flash( ex1, ex2, ex3 )
...
header.encoding.encodedSpace.fieldOfView_mm.x = ex1.acqp.ACQ_fov(1)*10;
header.encoding.encodedSpace.fieldOfView_mm.y = ex1.acqp.ACQ_fov(2)*10;
header.encoding.encodedSpace.matrixSize.x = nX;
header.encoding.encodedSpace.matrixSize.y = nY;

if(nZ>1)
    header.encoding.encodedSpace.fieldOfView_mm.z = ex1.acqp.ACQ_fov(3)*10;
...
if (ex1.method.PVM_EncPft(2)~=1)
    ...
    header.encoding.encodingLimits.kspace_encoding_step_1.minimum = ex1.method.PVM_EncSteps1(1) + round(nY/2);
    header.encoding.encodingLimits.kspace_encoding_step_1.maximum =   ex1.method.PVM_EncSteps1(end) + round(nY/2);
...
[ number_of_slices ] = get_number_of_slices( ex1 );
...
% end    
 ...
 if (number_of_slices>1)
    header.encoding.encodingLimits.slice.minimum = 0;
    header.encoding.encodingLimits.slice.maximum = ex1.method.PVM_SPackArrNSlices-1;
    header.encoding.encodingLimits.slice.center = 0;
    fprintf('number of slices is %d \n', ex1.method.PVM_SPackArrNSlices);
end

if(isfield(ex1.method,'PVM_NRepetitions'))
header.encoding.encodingLimits.repetition.minimum = 0;
header.encoding.encodingLimits.repetition.maximum = ex1.method.PVM_NRepetitions-1;
header.encoding.encodingLimits.repetition.center = 0;
fprintf('number of repetitions %d \n', ex1.method.PVM_NRepetitions);
else 
header.encoding.encodingLimits.repetition.minimum = 0;
...
if(isfield(ex1.method,'PVM_DwNDiffExp'))
header.encoding.encodingLimits.set.minimum = 0;
header.encoding.encodingLimits.set.maximum = ex1.method.PVM_DwNDiffExp-1;
header.encoding.encodingLimits.set.center = 0;
fprintf('number of echos diffusion%d \n', ex1.method.PVM_DwNDiffExp);
else    
header.encoding.encodingLimits.set.minimum = 0;
...
header.sequenceParameters.TR= ex1.method.PVM_RepetitionTime;
% header.sequenceParameters.TE= [ ex1.acqp.ACQ_echo_time, , ];

header.sequenceParameters.TE(1) = ex1.acqp.ACQ_echo_time;
header.sequenceParameters.TE(2) = ex2.acqp.ACQ_echo_time;
header.sequenceParameters.TE(3) = ex3.acqp.ACQ_echo_time;

%disp(['Echo Time: ' , num2str(ex.acqp.ACQ_echo_time) ]);
%  header.sequenceParameters.TE(2)= ;
%  header.sequenceParameters.TE(3)= ex3.acqp.ACQ_echo_time;
% header.sequenceParameters.TE(4)= ex.method.PVM_EchoTime+3;
header.sequenceParameters.flipAngle_deg=ex1.acqp.ACQ_flip_angle;
